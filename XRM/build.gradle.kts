...
repositories {
    ...
    // FP-23 NoteBlockAPI v1.7.0（koca2000/NoteBlockAPI，2026-03-25 发布，支持 Trumpet）
...
dependencies {
    ...
    compileOnly("com.magmaguy:EliteMobs:10.8.1")
    compileOnly("net.citizensnpcs:citizens-main:2.0.43-SNAPSHOT") {
        exclude(group = "*")
    }
    // FP-23 NoteBlockAPI v1.7.0（softdepend，编译期可见，运行时由服务器安装）
...
tasks.jar {
    archiveBaseName.set("XRM")
    archiveVersion.set(project.version.toString())
    archiveClassifier.set("")

    manifest {
        attributes(
            "Implementation-Title" to "暮澜纪元",
...
tasks.register<Copy>("deployPlugin") {
    from(tasks.jar)
    into("D:/xuanr/Desktop/燃烧之陨我的世界服务端/暮澜纪元我的世界MMORPG服务端/plugins")
...
tasks.withType<ProcessResources>().configureEach {
    ...
    val 版本字符串 = version.toString()
    filesMatching("plugin.yml") {
        expand(
            "version" to 版本字符串,
            "name" to "XRM",
            "description" to "暮澜纪元 MMORPG 核心插件",
            "author" to "玄锐暮"
        )
    }
}
